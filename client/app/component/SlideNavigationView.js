Ext.define('Ecommerce.component.SlideNavigationView', {
    ...
    config: {
        ...
        list: {
            width  : 250,
            maxDrag: null,
            itemTpl: '<i class="{icon}"></i> {title} <tpl if="notifications"><span class="notifications">{notifications}</span></tpl>',
            grouped: true,
            items  : [{
                xtype : 'toolbar',
                docked: 'top',
    ...
    initialize: function () {
        ...
        me.addCls('x-slidenavigation');

        me.list      = me.createNavigationList();
        ...
        me.createContainerCSS();

        var selectedItemIndex = 0;

        if (me.list.getStore().getCount() > 0) {
            Ext.each(me.list.getStore().getRange(), function (item, index) {
                if (item.get('selected') === true) {
                    selectedItemIndex = index;
                }
            });

            me.list.select(selectedItemIndex);
        }
    ...
    createSlideButton: function (el, config) {
        ...
        parent = config.selector ? el.down(config.selector) : undefined;
        if (parent) {
    ...
    onItemSelect   : function () {
        var me          = this,
            item        = arguments[1],
            onSelecting = typeof item.data.onSelecting == "function" ? item.data.onSelecting : null;

        if (onSelecting) {
            onSelecting.call(me, {
                args   : arguments,
                action : me.onSelect,
                failure: me.onSelectFailure,
                scope  : me
            });
        } else {
            me.onSelect.apply(me, arguments);
        }
    ...
    onSelect: function (list, item, eOpts) {
        ...
        if (Ext.isFunction(me._cache[index])) {
            ...
            Ext.defer(function () {
                me.list.select(me.container.getActiveItem().index || 0);
            }, 100);
        } else {
        ...
        if (!Ext.ModelManager.get(model)) {
            Ext.define(model, {
                ...
                config: {
                    idProperty: 'index',
                    fields    : [
                        'index', 'section', 'icon', 'title', 'group', 'selected', 'notifications', 'onSelecting', 'viewType',
                        {
                            name        : 'order',
                            defaultValue: 1
                        },
    ...
    closeContainer: function (duration) {
        var me       = this,
            duration = duration || me.config.slideDuration;
    ...
    openContainer: function (duration) {
        var me       = this,
            duration = duration || me.config.slideDuration,
            offsetX  = me.config.list.minWidth;
    ...
    toggleContainer: function (duration) {
        var me       = this,
            duration = Ext.isNumber(duration) ? duration : me.config.slideDuration;
        if (me.isClosed()) {
            me.openContainer(duration);
        } else {
            me.closeContainer(duration);
        }
    },
    ...
    createContainer: function () {
        var me                  = this,
            listPosition        = me.getListPosition(),
            containerSlideDelay = me.getContainerSlideDelay(),
            containerConstraint;
                        ...
                        animationend  : function (translatable, b, c) {
                            ...
                            if (me.isOpened()) {
                                ...
                                for (i = 0; i < hamburger.length; i++) {
                                    hamburger[i].className = 'hamburger-icon active';
                                }
                                ...
                                for (i = 0; i < hamburger.length; i++) {
                                    hamburger[i].className = 'hamburger-icon';
                                }
